...
st_write(mean_export, "./exported_data/geojson/madrid_mean_year_by_neighborhood.geojson", driver = "GeoJSON", delete_dsn = TRUE)
...
st_write(export, file_name_geojson, driver = 'GeoJSON', delete_dsn = TRUE)
```
...
weekly_mean <- aggregate(mean.NO2.concentration ~ week, df, mean)

above_limit <- weekly_mean$mean.NO2.concentration > 40
point_colors <- ifelse(above_limit, "#c1121f", "#0f4c5c")

par(mar = c(5, 5, 3, 1), bty = "l", las = 1)
plot(
  weekly_mean$week,
  weekly_mean$mean.NO2.concentration,
  type = "n",
  axes = FALSE,
  xlab = "Week",
  ylab = expression(paste("Mean NO"[2], " concentration (", mu, "g/m"^3, ")")),
  main = "General evolution of NO2 in 2023",
  ylim = c(min(weekly_mean$mean.NO2.concentration, 40, na.rm = TRUE) - 2,
           max(weekly_mean$mean.NO2.concentration, 40, na.rm = TRUE) + 2),
  xlim = c(1, 52)
)
axis(1, at = seq(1, 52, by = 4))
axis(2, las = 1)
box(col = "#3d5a80")
abline(h = 40, lty = 2, lwd = 1.5, col = "#c1121f")
grid(nx = NA, ny = NULL, col = "#d9e2ec", lty = "dashed", lwd = 0.8)
lines(weekly_mean$week, weekly_mean$mean.NO2.concentration, col = "#0f4c5c", lwd = 2.2)
points(
  weekly_mean$week,
  weekly_mean$mean.NO2.concentration,
  pch = 19,
...
sf::st_write(q_results[[quarter]], file_name_geojson, driver = "GeoJSON", delete_dsn = TRUE)
}
```
