...
# Configure global knitr chunk options for document execution.
knitr::opts_chunk$set(echo = TRUE)
```

```{r}
# Install/load required packages and project dependency helpers.
options(repos = c(CRAN = "https://cloud.r-project.org"))

required_cran <- c(
  "dplyr",  # used to arrange dataframe rows
  "tidyr",  # used to pivot a dataframe to create a summary table
  "sf",     # used for spatial analysis throughout the markdown
  "units",  # units of MB
  "rjson",  # read json in processing stage
  "stars",  # used for data cube analysis
  "gstat",  # used for interpolation
  "zyp",    # used to perform time series analysis
  "ggplot2",# used for enhanced cartographic plotting
  "ggspatial", # used to add OSM basemap tiles and map annotations
  "rosm"    # tile backend used by ggspatial
)
...
# load custom package from GitHub if it is not installed
if (!requireNamespace("widepollution", quietly = TRUE)) {
  if (!requireNamespace("remotes", quietly = TRUE)) {
    install.packages("remotes")
  }
  remotes::install_github("rodrigomalagon/widepollution", dependencies = TRUE)
}
...
output_dirs <- c(
  "./exported_data",
  "./exported_data/shapefiles",
  "./exported_data/geojson",
  "./exported_data/figures"
)
...
```{r}
# List monthly raw files and order them chronologically by month abbreviation.
...
```{r}
# Build the long-format yearly pollution table from monthly wide-format files.
monthly_data <- list()
...
```{r}
# Compute the percentage of validated measurements in the yearly dataset.
...
```{r}
# Check whether any NULL pollution measurements are present.
...
```{r}
# Summarize pollutant completeness against expected station-day-hour coverage.
...
```{r}
# Plot neighborhoods, highlighting stations with persistent missing data.
par(mar = c(2,2,0.5,0.5), mgp = c(0, 1, 0))
plot(neighborhoods$geometry,graticule=TRUE,col_graticule='gray',lwd = 0.1,col='lightblue',axes=TRUE,border=0)
...
We reshape our year analysed data into a dataframe and export it:
...
mean_export <- no2_mad_year |> sf::st_collection_extract(type = "POLYGON")
mean_export_wgs84 <- st_transform(mean_export, 4326)
st_write(mean_export, "./exported_data/shapefiles/madrid_mean_year_by_neighborhood.shp", delete_layer = TRUE)
st_write(mean_export_wgs84, "./exported_data/geojson/madrid_mean_year_by_neighborhood.geojson", driver = "GeoJSON", delete_dsn = TRUE)
...
```{r}
# Prepare a WGS84 layer for cartographic png display with OSM basemap
map_data <- st_transform(mean_export, 4326)
bbox <- st_bbox(map_data)
pad_x <- (bbox$xmax - bbox$xmin) * 0.06
pad_y <- (bbox$ymax - bbox$ymin) * 0.06
...
p_mean_map <- ggplot() +
  ...
  scale_fill_viridis_c(
    option = "C",
    direction = 1,
    name = expression("Annual mean NO"[2] * " (" * mu * "g/m"^3 * ")"),
  ...
  labs(
    title = "Madrid 2023 mean NO2 by neighborhood",
    subtitle = "Annual neighborhood average concentrations",
  ...
  theme(
    ...
    plot.caption = element_text(size = 8, color = "#4f4f4f", hjust = 1),
    ...
    legend.background = element_rect(fill = scales::alpha("white", 0.75), color = "#d9d9d9"),
    legend.key.height = grid::unit(0.9, "cm")
  )

print(p_mean_map)

# Direct export as PNG (independent of R Markdown knitting output)
ggsave(
  filename = "./exported_data/figures/madrid_mean_year_by_neighborhood_osm.png",
  plot = p_mean_map,
  width = 10,
  height = 8,
  units = "in",
...
We could recover the neighborhoods names by the id, as the aggregation step was done following the order of the `neighborhoods` sf object. 
Then, we can obtain the n (=10, 25) most polluted neighborhoods by name and plot them as well:

```{r}
# Extract and export the top-N most polluted neighborhoods by annual mean NO2.
n<-25
...
export_wgs84 <- st_transform(export, 4326)
#Write shapefile / GeoJSON
file_name_shp <- paste0('./exported_data/shapefiles/',n,'_most_polluted.shp')
file_name_geojson <- paste0('./exported_data/geojson/',n,'_most_polluted.geojson')
st_write(export, file_name_shp, delete_layer = TRUE)
st_write(export_wgs84, file_name_geojson, driver = 'GeoJSON', delete_dsn = TRUE)
```
...
```{r}
# Aggregate weekly NO2 datacube values to neighborhood-level weekly means.
aggregator <- st_transform(neighborhoods$geometry,crs=neighborhoods_crs)
...
```{r}
# Visualize weekly neighborhood NO2 distributions and EU threshold reference.
df <- data.frame(no2_neigh_week)

par(mar = c(5, 5, 3, 1), bty = "l", las = 1)
boxplot(
  mean.NO2.concentration ~ week,
  data = df,
  col = "#d9e2ec",
  border = "#3d5a80",
  outline = TRUE,
  ...
  main = "NO2 Concentration"
)
abline(h = 40, lty = 2, lwd = 1.5, col = "#c1121f")
grid(nx = NA, ny = NULL, col = "#edf2f7", lty = "dashed", lwd = 0.8)
```

Apply test on periods (quarters) and add neighborhood names
```{r}
# Run Mann-Kendall by quarter and attach neighborhood names to each result.
q_results <- list()
...
```{r}
# Inspect trend and p-value maps for a selected quarter.
quarter <- 4
...
```{r}
# Define helper rules to classify trend direction and significance levels.
...
Applying trend classification and creating shapefiles / GeoJSON

```{r}
# Apply trend classes to all quarters and export classified layers.
for (quarter in 1:4){
...
# writing shps / GeoJSON
file_name_shp <- paste0("exported_data/shapefiles/trend_classification_q_", quarter, ".shp")
file_name_geojson <- paste0("exported_data/geojson/trend_classification_q_", quarter, ".geojson")
q_results_wgs84 <- st_transform(q_results[[quarter]], 4326)
sf::st_write(q_results[[quarter]], file_name_shp, delete_layer = TRUE)
sf::st_write(q_results_wgs84, file_name_geojson, driver = "GeoJSON", delete_dsn = TRUE)
}
```
